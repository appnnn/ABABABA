...
namespace HR_Application.Repositories
{
    public class EmployeeRepository : IEmployeeRepository
    {
        public void AddEmployee(Employee employee)
        {
            ...
            employee.EmployeeID = GenerateNewEmployeeId();
        ...
        public DashboardInfoModel GetDashboardInfo()
        {
            ...
            //while (reader.Read())
            //{
            //    DashboardInfo.Add(new DashboardInfoModel
            //    {
            //        EmployeeCount = reader["TotalEmployees"] as int? ?? 0
            //        //PendingLeaves = reader["PendingLeaves"] as int? ?? 0,
            //        //PayrollsProceed = reader["PayrollsProceed"] as int? ?? 0
            //    });

            //}

            if (reader.Read()) // ✅ Use if instead of while since only one row is expected
            {
                //DashboardInfo.EmployeeCount = reader["TotalEmployees"] as int? ?? 0; // ✅ Assign to property
                DashboardInfo.EmployeeCount = Convert.ToInt32(reader["TotalEmployees"]);
            }
        ...
        public string GenerateNewEmployeeId()
        {
            ...
            var query = "SELECT TOP 1 EmployeeId FROM Employee ORDER BY EmployeeId DESC";
            ...
            int number = int.Parse(result.Substring(3));
            number++;
            return "EMP" + number.ToString("D4"); // EMP0002, EMP0003, etc.
